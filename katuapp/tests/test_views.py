...
class LoggedInTestCase(TestCase):
    ...
    def setUp(self):
        """テストメソッド実行前の事前設定"""
        # テストユーザーのパスワード
        self.password = 'Katu1218'

        # 各インスタンスメソッドで使うテスト用ユーザーを生成し、インスタンス変数に格納しておく
...
class TestDiaryCreateView(LoggedInTestCase):
    ...
    def test_create_diary_success(self):
        ...
        # 新規日記作成処理(POST)を実行
        response = self.client.post(reverse_lazy('katuapp:diary_create'), params)
        # 日記リストページへのリダイレクトを検証
        self.assertRedirects(response, reverse_lazy('katuapp:diary_list'))
        # 日記データがDBに登録されたかを検証
    ...
    def test_create_diary_failure(self):
        ...
        # 新規日記作成処理を実行
        response = self.client.post(reverse_lazy('katuapp:diary_create'))
...
class TestDiaryUpdateView(LoggedInTestCase):
    ...
    def test_update_diary_success(self):
        ...
        # 日記編集処理(POST)を実行
        response = self.client.post(reverse_lazy('katuapp:diary_update', kwargs={'pk': diary.pk}), params)
        # 日記詳細ページへのリダイレクトを検証
        self.assertRedirects(response, reverse_lazy('katuapp:diary_detail', kwargs={'pk': diary.pk}))
        # 日記データが編集されたかを検証
    ...
    def test_update_diary_failure(self):
        """日記編集処理が失敗することを検証する"""
        # 日記編集処理(POST)を実行
        response = self.client.post(reverse_lazy('katuapp:diary_update', kwargs={'pk': 999}))
...
class TestDiaryDeleteView(LoggedInTestCase):
    ...
    def test_delete_diary_success(self):
        ...
        # 日記削除処理(POST)を実行
        response = self.client.post(reverse_lazy('katuapp:diary_delete', kwargs={'pk': diary.pk}))
        # 日記リストページへのリダイレクトを検証
        self.assertRedirects(response, reverse_lazy('katuapp:diary_list'))
        # 日記データが削除されたかを検証
    ...
    def test_delete_diary_failure(self):
        """日記削除処理が失敗することを検証する"""
        # 日記削除処理(POST)を実行
        response = self.client.post(reverse_lazy('katuapp:diary_delete', kwargs={'pk': 999}))
